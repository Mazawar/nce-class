...
**第一册 2 课=1 单元（奇数课=课文、偶数课=单词语法）；二三四册每课一单元（笔记 PDF 逐课对应）：**

| 册 | 课数 | 单元数 | LRC/音频文件 | notes 粒度 |
|----|------|--------|--------------|-----------|
| NCE1 | 144 | 72 | NCE1-{单元}.lrc（单元粒度） | 001-002.json（单元粒度）|
| NCE2 | 96 | 96 | NCE2-{课}.lrc（课粒度） | 01.json（课粒度）|
| NCE3 | 60 | 60 | NCE3-{课}.lrc（课粒度） | 01.json（课粒度）|
| NCE4 | 48 | 48 | NCE4-{课}.lrc（课粒度） | 01.json（课粒度）|

- NCE1 单元 i = 课 (2i-1)（课文）+ 课 (2i)（单词语法）；NCE1 侧栏 `L1·2`，标题 "Lesson 1 & 2"
- NCE2/3/4 每课一单元：侧栏 `L5`，标题 "Lesson 5 · The Facts"
- 笔记 PDF 挂载: NCE1 Lesson N → 单元 (N+1)//2（奇数课有笔记）；NCE2/3/4 Lesson N → 单元 N（逐课对应）
